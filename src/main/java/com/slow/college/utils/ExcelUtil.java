...
public class ExcelUtil {
	...
	public static ArrayList<ArrayList<ArrayList<Object>>> readExcel2003(File file) {
		try {
			ArrayList<ArrayList<ArrayList<Object>>> rowList = new ArrayList<ArrayList<ArrayList<Object>>>();
			ArrayList<Object> colList;
			HSSFWorkbook wb = new HSSFWorkbook(new FileInputStream(file));
			Iterator<?> iter = wb.sheetIterator();

			//XSSFSheet sheet = wb.getSheetAt(0);
			while (iter.hasNext()) {
				ArrayList<ArrayList<Object>> rowListD = new ArrayList<ArrayList<Object>>();
				XSSFSheet sheet = (XSSFSheet) iter.next();
				XSSFRow row;
				XSSFCell cell;
				Object value;
				for (int i = sheet.getFirstRowNum(), rowCount = 0; rowCount < sheet.getPhysicalNumberOfRows(); i++) {
					...
					for (int j = row.getFirstCellNum(); j <= row.getLastCellNum(); j++) {
						...
						if (cell == null || cell.getCellType() == HSSFCell.CELL_TYPE_BLANK) {
							// 当该单元格为空
							if (j != row.getLastCellNum() && row.getFirstCellNum() == 0) {// 判断是否是该行中最后一个单元格
								colList.add("");
							}
							continue;
						}
					...
					if (colList != null && !colList.isEmpty()) {
						rowListD.add(colList);
					}
				} // end for i
				rowList.add(rowListD);
			}
			return rowList;
		} catch (Exception e) {
			return null;
	...
	public static void main(String[] args) {
		// 获取excel 文件
		File file = new File("D:/礼品册发放人员信息表-内部测试 - 副本.xlsx");
		ArrayList<ArrayList<ArrayList<Object>>> result = ExcelUtil.readExcel(file);
